 ...
 * to input a String
 */
public class SearchByUserIdDialog extends Dialog {

	private static final Log log = LogFactory.getLog(SearchByUserIdDialog.class);
	 ...
	 */
	public SearchByUserIdDialog(Shell parent) {
		super(parent);
	...
	protected Control createDialogArea(Composite parent){
    	...
    	comp.getShell().setText("Search by User ID");
	    ...
	    final Text findText = new Text(comp, SWT.DROP_DOWN);
	    	// default to user's id
	    userIdText = UserInfoBean.getInstance().getUserName();
	    findText.setText(userIdText);
	...
	//	int maxTerms = -1;
		...
		if(buttonId == 0){
			...
			TableComposite.getInstance().getBulkLoadStatus(null, userIdText).start();
			close();
		}
		//Cancel
		else if(buttonId ==1) {
