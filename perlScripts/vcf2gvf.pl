	...
	elsif ($line =~ m/^([^\t]+)\t([^\t]+)\t([^\t]+)\t([^\t]+)\t([^\t]+)\t([^\t]+)\t([^\t]+)\t([^\t]+)\t([^\t]+)\t(.+)/)
	{
		...
		foreach (@ALT_all)
		{
			...
			if (length($REF) == length($ALT) && length($REF) == 1)
			{
				...
				$variant_output[$ALT_index] = "$chromosome\t$source\t$varType\t$begin_gvf\t$end_gvf\t$score\t$strand\t$phase\tID=$varID;Reference_seq=$reference_gvf;Variant_seq=$variant_gvf;";
			}
